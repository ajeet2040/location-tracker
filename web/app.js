// Constants
let geonamesBaseUrl = 'https://api.geonames.org/searchJSON'
...
async function getLocationDetails(locationName) {
    let apiURL = `http://${geonamesBaseUrl}?q=${locationName}&maxRows=10&username=${geonamesApiUsername}`
...
function locationFinder(evt) {
    let locationName = evt.currentTarget.value
    if(locationName.length > 2){ 
        results.style.display = "block"
        getLocationDetails(locationName).then(item => {
            let matchingResult = item.geonames
